...
export const useStickersStore = create<StickersStore>((set, get) => ({
  ...
  addStickerToTimeline: async (iconName: string) => {
    ...
    try {
      ...
      // --- NEW: Upload sticker to local storage for persistence ---
      ...
      try {
        ...
        if (uploadResponse.ok) {
          ...
          if (uploadResult.success) {
            ...
            // The upload-local API currently doesn't return filePath in JSON, 
            // but it's encoded in the serve URL.
            // We can extract it or just rely on the persistent URL.
            // For reportState to work reliably, having filePath is best.
            const urlObj = new URL(uploadResult.url, window.location.origin);
            localFilePath = urlObj.searchParams.get("path") || "";
          }
        }
      } catch (uploadError) {
        console.warn("Failed to upload sticker for persistence, falling back to blob URL", uploadError);
      }
      // -----------------------------------------------------------
      ...
      const added = await addMediaFile(activeProject.id, mediaItem);

      if (!added) {
        throw new Error("Failed to add sticker to media store");
      }
